...
def main():
    ...
    # A3 z = 2.06m
    angle = 47 # angle of csat at A3 site, degree
    dates = pd.date_range('2022-08-30', '2025-12-31')  # modify based on the dat_in
    outfile = dat_out + f"A3_{dates[0].date()}_{dates[-1].date()}.xlsx"
    ...
    for date in dates:
        ...
        for idx, file in enumerate(matching_files):
            ...
            # read variables
            # A3 only have 1 layer measurements
            ux_all = ds['Ux']
            uy_all = ds['Uy']
            uz_all = ds['Uz']
            Ts_all = ds['Ts']
