...
def main():
    ...
    # A5 z = 2.13m
    angle = 57 # angle of csat at A5 site, degree
    dates = pd.date_range('2022-09-13', '2026-02-09')  # modify based on the dat_in
    outfile = dat_out + f"A5_{dates[0].date()}_{dates[-1].date()}.xlsx"
    ...
    for date in dates:
        ...
        for idx, file in enumerate(matching_files):
            ...
            # read variables
            # A5 only have 1 layer measurements
            ux_all = ds['Ux']
            uy_all = ds['Uy']
            uz_all = ds['Uz']
            Ts_all = ds['Ts']
